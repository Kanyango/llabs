...
var poly = {
	...
	create : function(req , res , next)
	{
		...
		req.app.db.models.Policy.create(fieldsToSet, 
			function(err ,  docs){
	...
	pols : function(req , res , next)
	{
		var id = req.body.id;
		console.log(req.body);

		req.app.db.models.Policy.update(
			  {_id : mongoose.Types.ObjectId(id)},
			  {$addToSet: {pols : req.body}},
			 function(err , docs){
				if(err)
		    	{
		    		return next(err);
	...
	read : function(req , res , next)
	{
		...
		req.app.db.models.Policy.find({user: req.payload._id},
		    function(err , docs)
			{
				if(err)
				{
	...
	remove : function(req , res , next)
	{
	  	...
	  	req.app.db.models.Policy.findByIdAndRemove(req.params.id, 
	  		function(err , info){
	  			if(err)
	  			{
	  				return next(err);
